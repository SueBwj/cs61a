...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        return 1
    return sum([num_trees(i)*num_trees(n-i) for i in range(n)])
...
def shuffle(cards):
    ...
    """
    # card = range(6)
    # 思路是正确的，但是在half_length使用的是len(cards)/2，如果只有一个/返回的是不是整数，会导致后面切片的错误。
    assert len(cards) % 2 == 0, 'len(cards) must be even'
    all_cards = list(cards)
    half_length = len(cards)//2
    half = all_cards[half_length:]
    shuffled = []
    for i in range(half_length):
        shuffled.append(all_cards[i])
        shuffled.append(half[i])
    return shuffled


def link_length(link):
    """the function is used to calculate the length of the link."""
    if link is Link.empty:
        return 0
    else:
        return 1 + link_length(link.rest)
...
def insert(link, value, index):
    ...
    if link is Link.empty:
        ...
        # 如果link  = Link(val,link) 则是创建一个local的link，并不等于传进来的link object
