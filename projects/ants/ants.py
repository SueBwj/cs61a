    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if (isinstance(place.ant, ContainerAnt)):
                if (place.ant.contained_ant != None or isinstance(place.ant.contained_ant, ContainerAnt)):
...
class ThrowerAnt(Ant):
    ...
    # min_range,max_range 用来表示这种类型的ant的攻击范围
    min_range = 0
    max_range = float('inf')
    ...
    def nearest_bee(self, beehive):
        ...
        move = 0
        while (checking_place != beehive):
            # 在该place有bee同时该地点在攻击的范围内
            if len(checking_place.bees) > 0 and self.min_range <= move <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # 这里的bee其实是链接到了原列表的对象，所以删除的是原列表的对象，而在复制的列表中遍历
        bees = checking_place.bees[:]
        for bee in bees:
...
class QueenAnt(ThrowerAnt):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        if (QueenAnt.queen_cnt == 1):
            ...
            while (pre_place != None):
                cur_ant = pre_place.ant
                if (cur_ant != None and cur_ant.double_flag == 0):
                    cur_ant.damage = cur_ant.damage*2
                    cur_ant.double_flag = 1
    ...
    def remove_from(self, place):
        """if it is impostor then you can remove it but if it is real queen then you can't remove it"""
        if (QueenAnt.queen_cnt > 1):
            ThrowerAnt.remove_from(self, place)
            QueenAnt.queen_cnt -= 1
...
class Bee(Insect):
    ...
    def __init__(self, armor):
        # 注意是作为instance的变量，不是作为类的变量，因为每个instance有不同的scared值，如果是在类里，一个instance改变了这个值，其他instance也会跟着改变
        Insect.__init__(self, armor)
        self.scared = False
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if (isinstance(self, ContainerAnt) and (self.contained_ant == None) and not isinstance(other, ContainerAnt)):
            return True
        else:
            return False
        # END Problem Optional 2
...
def apply_status(status, bee, length):
    ...
    def NEW_ACTION(gamestate):
        # 这里用nonlocal可以保证每一次使用后length的更新值可以继续使用
        nonlocal length
        if length > 0:
            # 如果length>0使用新action
    ...
    # bee的action赋新值保证length的更新记录得以保存！
