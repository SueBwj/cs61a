...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    # 要么返回数值，布尔值要么返回表达式（特殊值：and，or或者procedure）
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # 不是symbol（define, and or）是operator

        operator = scheme_eval(first, env)
        validate_procedure(operator)
        # 如果operator是加减乘除则对剩下的所有值进行操作
        if isinstance(operator, MacroProcedure):
            return scheme_eval(scheme_apply(operator, rest, env), env)
        # 对每个operand进行操作，化到最简
    ...
    def apply(self, args, env):
        ...
        if args != nil:
            while True:
                ...
                if not isinstance(args.rest, Pair):
                    if args.rest != nil:
                        python_args.append(args.rest)
                    break
                elif args.rest == nil:
                    break
                else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        Expressions = Pair(function_parameter, expressions.rest)
        env.define(function_name, do_lambda_form(Expressions, env))
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        # 确保语句的长度为2，first表示symbol，second表示value
        validate_form(clause, 2, 2)
        # 这里在Pair后面接上names和values非常巧妙
