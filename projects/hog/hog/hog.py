...
def swine_align(player_score, opponent_score):
    ...
    ans, cnt = 1, 1
    while cnt <= min(player_score, opponent_score):
        if player_score % cnt == 0 and opponent_score % cnt == 0:
            ans = max(ans, cnt)
        cnt += 1
    if ans >= 10:
        return True
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_highest(score0, score1):
        if (who == 0):
            gapping0 = score0-last_score
            if gapping0 > running_high:
                print(gapping0, "point(s)! The most yet for Player 0")
                return announce_highest(who, score0, gapping0)
            else:
                return announce_highest(who, score0, running_high)
        else:
            gapping1 = score1-last_score
            if gapping1 > running_high:
                print(gapping1, "point(s)! The most yet for Player 1")
                return announce_highest(who, score1, gapping1)
            else:
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def count_total_points(*args):
        cnt, result = trials_count, 0
        while cnt > 0:
            result += original_function(*args)
            cnt -= 1
        return result/trials_count
    return count_total_points
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    recorded_result = -1
    lowerest_num_dice = 1
    averge_roll_dice = make_averaged(roll_dice, trials_count)
    while num_dice <= 10:
        current_result = averge_roll_dice(num_dice, dice)
        if recorded_result < current_result:
            lowerest_num_dice = num_dice
            recorded_result = current_result
        num_dice += 1
    return lowerest_num_dice
    # END PROBLEM 9
