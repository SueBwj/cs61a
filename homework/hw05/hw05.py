...
class Mint:
    ...
    def create(self, kind):
        "*** YOUR CODE HERE ***"
        # 这个地方kind传进来的是一个class而不是instance，毕竟要用class来表示类型
        coin = kind(self.year)
        return coin
...
def store_digits(n):
    ...
    if (cal_digit(n) == 1):
        ...
        first_num = n//10**(cal_digit(n)-1)
        all_but_first = n-first_num*10**(cal_digit(n)-1)
        return Link(first_num, store_digits(all_but_first))
...
def bst_min(t):
    ...
    >>> bst_min(t2)
    1
    >>> t7 = Tree(2, [Tree(6, [Tree(5)]), Tree(4)])
    >>> bst_min(t7)
    2
    """
    res = []
    if t.is_leaf():
        ...
        for branch in t.branches:
            res.append(min([b.label for b in branch.branches]+[branch.label]))
        return min(min(res), t.label)
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 1:
            # 如果branch长度为1，父节点只有一个子节点也返回true
            return True
        # 中间节点要大于左边的最大值，小于右边的最小值，同时对于每一个子树都是如此
        left_max = bst_max(t.branches[0])
        right_min = bst_min(t.branches[1])
        if left_max <= t.label <= right_min:
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [t.label]
    for branch in t.branches:
        # 关键在于使用递归处理树的嵌套结构，以及在for循环中调用函数
